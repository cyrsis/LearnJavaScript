    ...
    internal sealed class Configuration : DbMigrationsConfiguration<EntityStore> {
        ...
        protected override void Seed(EntityStore context) {
            ...
            Model lamboAventador = new Model {
                ...
                Description = "The Aventador LP 700-4 represents a whole new level of performance, sets new benchmarks in the super sports car segment, and provides a glimpse into the future. It’s a car that’s already achieved legendary status.",
                EngineType = "V12, 60°, MPI",
                BreakHorsepower = 700,
                ZeroToSixty = 2.7m,
                TopSpeed = 217,
                BasePrice = 397500
            ...
            context.ModelImages.AddOrUpdate(x => new { x.ModelId, x.Order },
                new Models.ModelImage { ModelId = spiderId, ShortDescription = "Front", LongDescription = "Front", Order = 1, HighResolutionUrl = string.Format(spiderImageBase, "Front_HighRez"), LowResolutionUrl = string.Format(spiderImageBase, "Front_LowRez") },
                new Models.ModelImage { ModelId = spiderId, ShortDescription = "Rear", LongDescription = "Rear", Order = 2, HighResolutionUrl = string.Format(spiderImageBase, "Rear_HighRez"), LowResolutionUrl = string.Format(spiderImageBase, "Rear_LowRez") },
                new Models.ModelImage { ModelId = spiderId, ShortDescription = "Side", LongDescription = "Side", Order = 3, HighResolutionUrl = string.Format(spiderImageBase, "Side_HighRez"), LowResolutionUrl = string.Format(spiderImageBase, "Side_LowRez") },
                new Models.ModelImage { ModelId = spiderId, ShortDescription = "Top", LongDescription = "Top", Order = 4, HighResolutionUrl = string.Format(spiderImageBase, "Top_HighRez"), LowResolutionUrl = string.Format(spiderImageBase, "Top_LowRez") },
                new Models.ModelImage { ModelId = aventadorId, ShortDescription = "Front", LongDescription = "Front", Order = 1, HighResolutionUrl = string.Format(aventadorImageBase, "Front_HighRez"), LowResolutionUrl = string.Format(spiderImageBase, "Front_LowRez") },
                new Models.ModelImage { ModelId = aventadorId, ShortDescription = "Rear", LongDescription = "Rear", Order = 2, HighResolutionUrl = string.Format(aventadorImageBase, "Rear_HighRez"), LowResolutionUrl = string.Format(spiderImageBase, "Rear_LowRez") },
                new Models.ModelImage { ModelId = aventadorId, ShortDescription = "Side", LongDescription = "Side", Order = 3, HighResolutionUrl = string.Format(aventadorImageBase, "Side_HighRez"), LowResolutionUrl = string.Format(spiderImageBase, "Side_LowRez") }
            );

            //Order
            const string username = "TestAccount";
            context.Orders.AddOrUpdate(x => new { x.Username, x.MakeId, x.ModelId },
                new Order { Username = username, MakeId = ferrariId, ModelId = spiderId, Date = DateTime.Now.AddDays(-7), TotalCharge = 280465 }
            );
